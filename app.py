...
def check_rate_limit(ip, max_attempts=5, window=300, scope="login"):
    """Bloqueia após max_attempts em `window` segundos. Usa Redis se disponível.
    `scope` separa contadores distintos (ex.: 'login' vs 'register')."""
    mem_key = f"{scope}:{ip}"
    r = _get_redis()
    if r:
        try:
    ...
    now = time.time()
    if mem_key in login_attempts:
        data = login_attempts[mem_key]
        if now - data["last"] > window:
            login_attempts[mem_key] = {"count": 0, "last": now}
            return True
...
def record_login_attempt(ip, window=300, scope="login"):
    mem_key = f"{scope}:{ip}"
    r = _get_redis()
    if r:
        try:
    ...
    # Fallback memória
    now = time.time()
    if mem_key not in login_attempts:
        login_attempts[mem_key] = {"count": 0, "last": now}
    login_attempts[mem_key]["count"] += 1
    login_attempts[mem_key]["last"] = now


def reset_login_attempts(ip, scope="login"):
    mem_key = f"{scope}:{ip}"
    r = _get_redis()
    if r:
        try:
            r.delete(f"ratelimit:{scope}:{ip}")
            return
        except Exception:
            pass
    if mem_key in login_attempts:
        del login_attempts[mem_key]
...
def register():
    error = ""
    # Kill switch de emergência: defina DISABLE_REGISTRATION=1 no Railway para
    # suspender cadastros imediatamente (sem novo deploy). Útil contra ataque em curso.
    if os.getenv("DISABLE_REGISTRATION", "").strip() == "1":
        content = """<div class="auth-container"><div class="auth-card" style="text-align:center">
            <h2>Cadastros temporariamente suspensos</h2>
            <p style="color:var(--text2);margin:16px 0">Estamos em manutenção do sistema de cadastro.
            Tente novamente mais tarde ou fale com <a href="mailto:contato@atendente.online">contato@atendente.online</a>.</p>
            <a href="/" class="btn btn-secondary">Voltar</a></div></div>"""
        return base_html("Cadastro suspenso", content)
    if request.method == "POST":
        client_ip = request.remote_addr or "unknown"
        # Honeypot anti-bot: o campo 'website' fica escondido no form; humanos nunca preenchem.
        if request.form.get("website", "").strip():
            safe_log(f"[REGISTER] Honeypot acionado (provável bot) IP={client_ip}", level="WARN")
            record_login_attempt(client_ip, window=900, scope="register")
            return redirect("/verify-email")  # finge sucesso para não sinalizar ao bot
        ...
        # Rate limit anti-abuso: evita email bombing via /register (cada POST dispara email).
        if not check_rate_limit(client_ip, max_attempts=5, window=900, scope="register"):
            error = "Muitas tentativas de cadastro deste dispositivo. Aguarde 15 minutos e tente novamente."
            ...
            if not pw_ok:
                ...
                if existing and existing["email_verified"]:
                    ...
                    if existing and not existing["email_verified"]:
                        # Reaproveita a conta não verificada em vez de DELETE+INSERT.
                        # A tentativa anterior já criou linhas em consent_log referenciando
                        # este user; com PRAGMA foreign_keys=ON, o DELETE falhava
                        # (FOREIGN KEY constraint failed) e derrubava o /register.
                        db.execute(
                            """UPDATE users SET password_hash=?, name=?, company=?, plan=?,
                               plan_status='trial', msgs_limit=?, trial_ends_at=?, email_verified=0
                               WHERE id=?""",
                            (hash_password(password), name, company, plan, msgs_limit, trial_end, existing["id"])
                        )
                    else:
            ...
            for uid in safe_ids:
                try:
                    ...
                    for child_sql in (
                        ...
                        "DELETE FROM quick_replies WHERE user_id=?",
                        "DELETE FROM blocked_contacts WHERE user_id=?",
                        "DELETE FROM api_usage_log WHERE user_id=?",
                        "DELETE FROM product_gallery WHERE user_id=?",
                        "DELETE FROM payments WHERE user_id=?",
                    ):
                        try:
                            db.execute(child_sql, (uid,))
                        except sqlite3.OperationalError:
